...
class PreProcessat:
    def __init__(self,path):
        ...
        self.map_train_label_indices={label:np.flatnonzero(self.labels_train==np.where(self.classes==label)) for label in self.classes}
    ...
    def get_triplets_batch(self,n):
        idxs_a,idxs_p,idxs_n,key=[],[],[],[]
        for _ in range(n):
            label_l,label_r=np.random.choice(self.classes,2,replace=False)
            ...
            idxs_p.append(p)
            idxs_n.append(n)
            key.append((label_l,label_r))
        return self.images_train[idxs_a,:],self.images_train[idxs_p,:],self.images_train[idxs_n,:],key